...
def record_audio(duration=5, samplerate=44100):
    try:
        ...
        return ("Recorded Audio", audio_buffer.getvalue())
    except Exception as e:
        return ("Error", f"Error recording audio: {str(e)}")
...
def transcribe_audio(audio_file):
    ...
    return result.get("text", "No transcription output.")
...
def process_transaction_message(message):
    ...
    input_prompt = f"{system_prompt}\nMessage: {message}"
    response = rag_llm.invoke(input_prompt)
    return response.content if hasattr(response, 'content') else response

# Gradio UI
with gr.Blocks() as app:
    gr.Markdown("# 🎧 Audio Transaction Processor")
    
    with gr.Tab("Record Audio"):
        duration = gr.Slider(minimum=1, maximum=10, value=5, label="Recording Duration (seconds)")
        record_button = gr.Button("🎤 Record Audio")
        recorded_audio = gr.Audio(label="Recorded Audio", interactive=False)
        record_button.click(record_audio, inputs=[duration], outputs=[recorded_audio])
    
    with gr.Tab("Upload Audio"):
        uploaded_audio = gr.File(label="Upload an Audio File")
    
    transcribe_button = gr.Button("🎤 Transcribe Audio")
    transcription_output = gr.Textbox(label="Transcription", interactive=True)
    transcribe_button.click(transcribe_audio, inputs=[uploaded_audio], outputs=[transcription_output])
    
    process_button = gr.Button("💼 Process Transaction Details")
    processed_output = gr.Code(label="Extracted Transaction Details", language="json")
    process_button.click(process_transaction_message, inputs=[transcription_output], outputs=[processed_output])
